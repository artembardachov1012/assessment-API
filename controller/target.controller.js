...
const {
  getTargets: redisGetTargets,
  getTargetById: redisGetTargetById,
  getUtcDate,
  getUtcHour,
...
exports.createTarget = async (req, res) => {
  try {
    ...
    const target = await redisClient.set(
      "TARGETS",
      JSON.stringify([req.body, ...targets])
    );
    ...
    if (!target) {
      ...
      if (!score) {
        await redisClient.set(
          "SCORE",
          JSON.stringify({ [utcDate]: { [req.params.id]: 1 } })
        );
      } else {
        ...
        if (!scoreDateObj) {
          ...
          if (scoreDateObj[req.params.id]) {
            console.log(
              "IF WE DON HAVE DATE AND ALSO ID OF TARGET THAN INCREASE NUMBER OF ATTEMPTS"
            );
            ...
            console.log(
              "IF WE DON HAVE DATE BUT NOT ID OF TARGET THAN SET ATTEMP AS 1"
            );
...
exports.getDecision = async (req, res) => {
  try {
    ...
    const targets = await redisGetTargets();
    if (targets.length) {
      const targetWithGeoState = targets.filter(
        (target) =>
          target["accept"]["geoState"]["$in"].indexOf(userGeoState) >= 0 &&
          target["accept"]["hour"]["$in"].indexOf(utcHour) >= 0
      );
      if (targetWithGeoState.length) {
        const sortedTargets = targetWithGeoState.sort(
